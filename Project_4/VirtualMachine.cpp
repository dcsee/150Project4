...
TVMStatus VMDirectoryOpen(const char *dirName, int *dirDescriptor){
	...
	*dirDescriptor = fatController->openDirectory(dirName);
	printf("VMDirectoryOpen(): opened directory '%s' with descriptor %d\n", dirName, *dirDescriptor);
	MachineResumeSignals(&sigState);
...
TVMStatus VMDirectoryRead(int dirDescriptor, SVMDirectoryEntryRef dirEnt){
	...
	if(dir != NULL){
		*dirEnt = *dir;
		...
		printf("VMDirectoryRead(): could not read from directory\n");
...
TVMStatus VMDirectoryClose(int dirDescriptor){
	...
	printf("VMDirectoryClose()\n");

	if(fatController->closeDirectory(dirDescriptor)){
		MachineResumeSignals(&sigState);
		return VM_STATUS_SUCCESS;
	}
...
TVMStatus VMDirectoryRewind(int dirDescriptor){
	...
	if(fatController->rewindDirectory(dirDescriptor)){	
		MachineResumeSignals(&sigState);
		return VM_STATUS_SUCCESS;
	}
...
TVMStatus VMDirectoryCurrent(char *absPath){
	//this is basically pwd
	TMachineSignalState sigState;	
	MachineSuspendSignals(&sigState);
	ThreadStore* tStore = ThreadStore::getInstance();
	FATController *fatController = tStore->getFATController();
	
	printf("VMDirectoryCurrent()\n");
	
	if(absPath == NULL){
		MachineResumeSignals(&sigState);
		printf("VMDirectoryCurrent() - abspath was null.\n");
		return VM_STATUS_ERROR_INVALID_PARAMETER;
	}

	if(fatController->getCurrentDirectoryName() == NULL){
		MachineResumeSignals(&sigState);
		printf("VMDirectoryCurrent() - directory name was null.\n");
		return VM_STATUS_ERROR_INVALID_PARAMETER;
	}

	const char* newPath = fatController->getCurrentDirectoryName();

	for(int i = 0; i < VM_FILE_SYSTEM_MAX_PATH; i++){	//copy the string into absPath,
		absPath[i] = newPath[i];												//terminate if and when 256 characters
		if(newPath[i] == '\0'){													//are copied
			break;
		}
	}
...
TVMStatus VMDirectoryChange(const char* path){
	...
	if(path == NULL){
		MachineResumeSignals(&sigState);
		printf("VMDirectoryChange() - path was null.\n");
		return VM_STATUS_ERROR_INVALID_PARAMETER;
	}

	fatController->changeDirectory(path);
	MachineResumeSignals(&sigState);
	return VM_STATUS_SUCCESS;
...
TVMStatus VMStart(int tickms, TVMMemorySize heapSize, int machinetickms, TVMMemorySize sharedSize, const char *mount, int argc, char* argv[]){
	...
	//create FAT controller last
	tStore->createFATController(mount);					//creates object to interface with disk image
