...
//	printf("FATController::readDirectory()\n");
	...
	if(dirDescriptor < (int)directories->size()){
		...
		printf("FATController::readDirectory() - dirDescriptor %d exceeded range %d\n", dirDescriptor, directories->size());
		return NULL;
	}
}
...
bool FATController::closeDirectory(int dirDescriptor){
	...
	printf("FATController::closeDirectory()\n");
...
//			int rootDirIndex = findRootEntryByName(dirName);
	...
	//		make a struct of all this data, stick it in array at id = newid
	//		return the id

	}

	//first, check if this directory is already open. if not,
	//
	//	if current location is root,
	//
	return 0;
}
...
FATController* ThreadStore::getFATController(){
